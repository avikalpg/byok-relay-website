...
const CPU_OVERHEAD = {
  ...
  runs: 10000,
  warmup: 500,
  ms: { min: 0.013, p50: 0.014, p90: 0.021, p99: 0.041, p999: 0.290, max: 0.500, mean: 0.017 },
};
...
function Logo() {
  return (
    <Link to="/" className="flex items-center gap-2">
      <img src="/android-chrome-192x192.png" alt="byok-relay logo" width="28" height="28" className="h-7 w-7" />
...
function OverheadTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ value: number }>; label?: string }) {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto max-w-7xl px-6 pt-16 pb-20">
          <Link to="/" className="mb-8 inline-flex items-center gap-2 font-mono text-[11px] text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 font-mono text-[11px] tracking-tight">
            ...
            <span className="text-muted-foreground">real measurements · reproducible · open methodology</span>
          </div>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground">
            The relay adds exactly three operations between your app and the AI provider: a token
            hash, a SQLite lookup, and an AES-256-GCM decryption. Here's what that costs, measured.
          </p>
        </div>
      </section>
      ...
      <div className="border-b border-border bg-muted/30">
        <div className="mx-auto max-w-7xl px-6 py-3">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-1 font-mono text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1.5"><Server className="h-3 w-3" /> Linux x64 · Node v22.22.1 · better-sqlite3 WAL</span>
            <span className="flex items-center gap-1.5"><Clock className="h-3 w-3" /> {CPU_OVERHEAD.runs.toLocaleString()} runs · {CPU_OVERHEAD.warmup} warmup · process.hrtime.bigint()</span>
            <span className="flex items-center gap-1.5"><Zap className="h-3 w-3" /> Measured: {CPU_OVERHEAD.date}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-16 space-y-20">
        ...
        <section>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard label="p50 latency" value={CPU_OVERHEAD.ms.p50.toString()} unit="ms" sub="14 µs median" />
            <StatCard label="p90 latency" value={CPU_OVERHEAD.ms.p90.toString()} unit="ms" sub="21 µs" />
            <StatCard label="p99 latency" value={CPU_OVERHEAD.ms.p99.toString()} unit="ms" sub="41 µs" />
            <StatCard label="p99.9 latency" value={CPU_OVERHEAD.ms.p999.toString()} unit="ms" sub="290 µs worst-tail" />
          </div>
          <p className="mt-4 font-mono text-[12px] text-muted-foreground">
            ↳ All five relay operations — HMAC hash, two SQLite reads, AES-GCM decrypt, JSON re-serialise — complete in under{" "}
            <span className="text-foreground font-medium">50 µs</span> at p99. The network hop to the AI provider dominates.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={PERCENTILE_DATA} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="oklch(0.88 0.015 80 / 0.4)" vertical={false} />
                  <XAxis
                    dataKey="label"
                    tick={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, fill: "oklch(0.45 0.015 70)" }}
                  ...
                  <YAxis
                    ...
                    domain={[0, 0.6]}
                  />
                  <Tooltip content={<OverheadTooltip />} cursor={{ fill: "oklch(0.88 0.015 80 / 0.3)" }} />
                  <ReferenceLine y={5} stroke="oklch(0.7 0.21 45 / 0.4)" strokeDasharray="4 3"
                    label={{ value: "5ms target", position: "insideTopRight", fontFamily: "JetBrains Mono, monospace", fontSize: 10, fill: "oklch(0.7 0.21 45)" }} />
            ...
            <p className="mt-4 font-mono text-[11px] text-muted-foreground">
              All percentiles are well below the 5ms overhead target. The p99.9 spike (0.29ms) is OS scheduling jitter, not relay work.
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="rounded-xl border border-border bg-card p-6">
            ...
            <p className="mb-6 font-mono text-[11px] text-muted-foreground">
              byok-relay CPU overhead (p99) vs typical provider time-to-first-token (p50, indicative)
            </p>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  ...
                  layout="vertical"
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="oklch(0.88 0.015 80 / 0.4)" horizontal={false} />
                  <XAxis
                    type="number"
                    scale="log"
                    domain={[0.01, 2000]}
                    tickFormatter={(v) => v < 1 ? `${v}ms` : `${v}ms`}
                    tick={{ fontFamily: "JetBrains Mono, monospace", fontSize: 10, fill: "oklch(0.45 0.015 70)" }}
                    axisLine={false} tickLine={false}
                  />
                  <YAxis
                    type="category"
                  ...
                  <Bar dataKey="ms" radius={[0, 4, 4, 0]} maxBarSize={32}>
                    {PROVIDER_LATENCY_CONTEXT.map((entry, i) => (
                      <Cell key={i} fill={entry.isMeasured ? "oklch(0.7 0.21 45)" : "oklch(0.45 0.015 70 / 0.5)"} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <p className="mt-3 font-mono text-[11px] text-muted-foreground">
              The relay's CPU overhead is 4–5 orders of magnitude smaller than the AI model's response time.
              Even accounting for the relay→provider network hop (typically 1–5ms in the same region), the
              total overhead is under 0.5% of a typical AI call.
            </p>
          </div>
        </section>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              ...
              <ol className="space-y-3">
                {[
                  { n: "1", label: "HMAC-SHA256 token hash", detail: "Incoming relay token → SHA-256 digest for DB lookup" },
                  { n: "2", label: "SQLite user lookup", detail: "Indexed SELECT on token_hash column (WAL mode)" },
                  { n: "3", label: "SQLite key lookup", detail: "Indexed SELECT on (user_id, provider) unique index" },
                  { n: "4", label: "AES-256-GCM decrypt", detail: "Recover user's provider API key from ciphertext" },
                  { n: "5", label: "JSON re-serialise", detail: "Stringify request body before upstream forward" },
                ].map(op => (
                  <li key={op.n} className="flex gap-3">
                    <span className="font-mono text-[11px] text-signal mt-0.5 shrink-0">{op.n}.</span>
                    <div>
                      <p className="font-mono text-[12px] font-medium text-foreground">{op.label}</p>
            ...
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              <h3 className="font-mono text-sm font-medium">What's NOT included in these numbers</h3>
              <div className="space-y-3">
                {[
                  { icon: "🌐", label: "Client → relay network", detail: "Depends on your users' location and relay host region." },
                  { icon: "🏃", label: "Relay → provider network", detail: "Typically 1–5ms if relay is co-located in the same cloud region as the AI provider." },
                  { icon: "🤖", label: "AI model inference", detail: "Hundreds to thousands of ms depending on model and prompt length. Dominates all other costs." },
                  { icon: "📦", label: "Response streaming", detail: "The relay pipes the response stream without buffering — no added latency on the streaming path." },
                ].map(item => (
                  <div key={item.label} className="flex gap-3">
                    <span className="text-base shrink-0 mt-0.5">{item.icon}</span>
                    <div>
                      <p className="font-mono text-[12px] font-medium text-foreground">{item.label}</p>
              ...
              <div className="mt-4 rounded-lg border border-signal/30 bg-signal/5 p-3">
                <div className="flex gap-2">
                  ...
                  <p className="font-mono text-[11px] text-muted-foreground">
                    <span className="text-foreground font-medium">Network tip:</span> Host your relay in the same
                    cloud region as your primary AI provider (e.g. us-east-1 for OpenAI). The relay→provider
                    hop then adds &lt;2ms.
                  </p>
                </div>
              </div>
        ...
        <section>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="flex items-center gap-2 border-b border-border px-4 py-3 bg-muted/30">
              ...
              <span className="font-mono text-[12px] text-muted-foreground">Run the benchmark locally</span>
            </div>
            <pre className="overflow-x-auto p-5 text-[12px] leading-relaxed"><code className="font-mono text-foreground">{`# Clone the repo
...
#   p90:  0.021 ms
#   p99:  0.041 ms
#   p999: 0.290 ms`}</code></pre>
          </div>
          <p className="mt-4 font-mono text-[11px] text-muted-foreground">
            The benchmark script lives at{" "}
            <a href={`${GH_URL}/blob/main/scripts/bench-cpu.js`} target="_blank" rel="noreferrer"
            ...
            measures {CPU_OVERHEAD.runs.toLocaleString()} requests using <code className="text-foreground">process.hrtime.bigint()</code> for nanosecond precision.
          </p>
        </section>
        ...
        <section className="rounded-xl border border-border bg-card p-8 text-center">
          ...
          <p className="mt-2 font-mono text-sm text-muted-foreground">
            The relay adds &lt;0.05ms of CPU overhead. Your users' AI credits, your zero inference bill.
          </p>
      ...
      <footer className="border-t border-border py-8">
        <div className="mx-auto max-w-7xl px-6">
          <p className="font-mono text-[11px] text-muted-foreground text-center">
            byok-relay · Apache 2.0 · benchmarks run {CPU_OVERHEAD.date} · methodology is open and reproducible
          </p>
        </div>
      </footer>
    </div>
